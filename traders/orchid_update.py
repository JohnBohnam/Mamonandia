...
class Trader:
    ...
    def order_orchid(self, state: TradingState):
        ...
        min_profitable_bid = round(observation.askPrice + observation.importTariff + observation.transportFees + self.min_bid_margin)
        ...
        if available_q < -len(margin_values):
            for margin in margin_values:
                small_q = -1
                ask_price = self.get_orchid_price(state, margin)
    ...
    def order_gift_basket(self, state):
        ...
        res_buy = mid_price['GIFT_BASKET'] - mid_price['ROSES'] - mid_price['CHOCOLATE'] * 4 - mid_price[
            'STRAWBERRIES'] * 6 - 390
        res_sell = mid_price['GIFT_BASKET'] - mid_price['ROSES'] - mid_price['CHOCOLATE'] * 4 - mid_price[
            'STRAWBERRIES'] * 6 - 390
